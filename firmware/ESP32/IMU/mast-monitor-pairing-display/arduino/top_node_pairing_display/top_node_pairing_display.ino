...
// Seeed XIAO nRF52840 Sense IMU at the top of the mast.
LSM6DS3 myIMU(I2C_MODE, 0x6A);

// BLE service/characteristic for top node -> base bridge.
...
void writeStatus() {
  String status =
    String("filterReady=") + (filterReady ? "1" : "0") + "," +
...
void setup() {
  ...
  BLE.setLocalName(TOP_NODE_NAME);
  BLE.setDeviceName(TOP_NODE_NAME);
  ...
  writeStatus();
  BLE.advertise();
  Serial.println("Top node advertising as XIAO_TOP_IMU");
}
